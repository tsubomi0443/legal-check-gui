...
const generateDiffChunks = (diffResult) => {
  const chunks = [];
...
export default function App() {
  ...
  const [selectedFileId, setSelectedFileId] = useState('file-3');
  ...
  // --- ファイルツリー操作 ---
  const toggleFolder = (folderId, currentFiles) => {
  ...
  // --- 提案適用 ---
  const handleApplySuggestion = (chunkId, originalText, suggestedText) => {
    setContents(prev => {
  ...
  const handleFileUpload = (e) => {
    ...
    const newFileNode = { id: newId, name: uploadedFile.name, type: isPdf ? 'pdf' : 'txt' };
  ...
  const getSelectedFile = (nodes, id) => {
  ...
  const activeFile = isFreeMode 
    ? { id: 'free-mode', name: '自由記述（ファイル未指定）', type: 'txt' }
